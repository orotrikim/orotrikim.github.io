...
export function Header() {
  ...
  // The "Mobile-Proof" Scroll Logic
  const scrollToSection = (id: string) => {
    // 1. Start closing the menu
    setMobileMenuOpen(false);

    // 2. Identify if we are on the Home Page
    const isHomePage = window.location.hash === "#/" || window.location.hash === "" || window.location.pathname === "/";

    if (!isHomePage) {
      // IF ON /TEAM OR OTHER PAGE:
      navigate("/");
      // Mobile needs time to mount the homepage AND for the menu to slide out
      setTimeout(() => {
      ...
      // We must wait for the mobile menu animation to finish (approx 300ms)
      // otherwise the browser scrolls to the wrong position or doesn't scroll at all.
      setTimeout(() => {
        const element = document.getElementById(id);
        if (element) {
          // 'block: start' ensures it doesn't get stuck behind the fixed header
          element.scrollIntoView({ behavior: "smooth", block: "start" });
        }
      }, 350); 
    }
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {navItems.map((item) => (
              item.path ? (
                <Link key={item.id} to={item.path} className="text-[#F7F7F7]/80 hover:text-[#FFFF00] text-sm font-bold uppercase tracking-wider">
                  {item.label}
                </Link>
              ) : (
                <button key={item.id} onClick={() => scrollToSection(item.id)} className="text-[#F7F7F7]/80 hover:text-[#FFFF00] text-sm font-bold uppercase tracking-wider cursor-pointer bg-transparent border-none">
                  {item.label}
                </button>
              )
            ))}
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          ...
          >
            <div className="px-4 py-8 space-y-6 flex flex-col items-start">
              {navItems.map((item) => (
                item.path ? (
                  <Link
                    key={item.id}
                    ...
                    className="block w-full text-left text-[#F7F7F7] text-lg font-bold uppercase tracking-widest py-2"
                  >
                    {item.label}
                  </Link>
                ) : (
                  <button
                    ...
                    className="block w-full text-left text-[#F7F7F7] text-lg font-bold uppercase tracking-widest py-2 cursor-pointer bg-transparent border-none"
                  >
                    {item.label}
                  </button>
                )
